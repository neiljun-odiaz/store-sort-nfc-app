        ...
        "findDevices": function (cb) {
            ...
            wait_for_passive_target(device, function (rc, tag_type, tag_id) {
                ...
                tag.read(device, function (rc, ndef) {
                    ...
                    // Add NFC Card Tag ID on the returned object
                    if (tag.tag_id && ndef_obj.ndef.length > 0) {
                        ndef_obj.ndef[0]["tag_id"] = UTIL_BytesToHex(tag.tag_id);
                        ndef_obj.ndef[0]["new_card"] = false;
                    }

                    // This is for Empty/New Cards
                    if (ndef_obj.ndef.length == 0) {
                        ndef_obj.ndef.push({
                            tag_id: UTIL_BytesToHex(tag.tag_id),
                            new_card: true
                        });
                    }
